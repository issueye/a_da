...
type TabType = 'prompts' | 'workspace' | 'global' | 'builtins'
...
export function PluginsDialog({ store }: { store: AgentStore }) {
  ...
  const refreshList = async () => {
    setLoading(true)
    try {
      const [pluginItems, promptItems] = await Promise.all([
            ...
            {tab === 'prompts' ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                  ...
                  <text style={{ fontSize: 11, color: C.faint }}>
                    以下工具由 Agent 引擎内置托管，具备只读白名单校验与文件写入审批安全闸门。
                  </text>
                </div>

                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {BUILTIN_TOOLS_METADATA.map((bt) => (
                    <div
                      key={bt.name}
                      style={{
                        display: 'flex',
                      ...
                      >
                        ...
                        <div
                          style={{
                            ...
                            height: 18,
                            borderRadius: 4,
                            backgroundColor: bt.isReadOnly ? '#10b98122' : '#f59e0b22',
                            display: 'flex',
                            alignItems: 'center',
                          }}
                        >
                          <text
                            style={{
                              fontSize: 10.5,
                              color: bt.isReadOnly ? C.success : C.accent,
                              fontWeight: 600,
                            }}
                          >
